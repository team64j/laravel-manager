<template>
  <panel v-bind="$attrs" id="modules" class="py-3" @action="action">
    <template #header v-if="msg">
      <div class="alert alert-blue mb-3 mx-4" v-html="$root.lang('module_management_msg')"/>
    </template>

    <template #top>
      <router-link :to="{ name: 'Module', params: { id: null } }" class="btn-sm btn-green mr-1" role="button">
        <i class="fa fa-plus-circle fa-fw"/>
        <span class="hidden md:inline-block pl-1">{{ $root.lang('new_module') }}</span>
      </router-link>
  ...
  data () {
    return {
      ...
      actions: {
        ...
        delete: {
          icon: 'fa fa-trash fa-fw text-danger hover:text-rose-600',
  ...
  methods: {
    ...
    additional () {
      ...
      if (this.$attrs.columns) {
        ...
        if (this.$root.hasPermissions('new_module')) {
          actions.copy = this.actions.copy
        }

        if (this.$root.hasPermissions('edit_module')) {
          actions.disabled = this.actions.disabled
        }

        if (this.$root.hasPermissions('delete_module')) {
