<template>
  <panel v-bind="$attrs" id="snippets" class="py-3" @action="action">
    <template #header v-if="msg">
      <div class="alert alert-blue mb-3 mx-4" v-html="$root.lang('snippet_management_msg')"/>
    </template>

    <template #top>
      <router-link :to="{ name: 'Snippet', params: { id: null } }" class="btn-sm btn-green mr-1" role="button">
        <i class="fa fa-plus-circle fa-fw"/>
        <span class="hidden md:inline-block pl-1">{{ $root.lang('new_snippet') }}</span>
      </router-link>
  ...
  methods: {
    ...
    additional () {
      ...
      if (this.$attrs.columns) {
        ...
        if (this.$root.hasPermissions('new_snippet')) {
          actions.copy = this.actions.copy
        }

        if (this.$root.hasPermissions('edit_snippet')) {
          actions.disabled = this.actions.disabled
        }

        if (this.$root.hasPermissions('delete_snippet')) {
