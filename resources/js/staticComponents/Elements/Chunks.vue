<template>
  <panel v-bind="$attrs" id="chunks" class="py-3" @action="action">
    <template #header v-if="msg">
      <div class="alert alert-blue mb-3 mx-4" v-html="$root.lang('htmlsnippet_management_msg')"/>
    </template>

    <template #top>
      <router-link :to="{ name: 'Chunk', params: { id: null } }" class="btn-sm btn-green mr-1" role="button">
        <i class="fa fa-plus-circle fa-fw"/>
        <span class="hidden md:inline-block pl-1">{{ $root.lang('new_htmlsnippet') }}</span>
      </router-link>
      ...
      actions: {
        ...
        disabled: {
          values: {
            ...
            1: {
              icon: 'far fa-check-circle fa-fw text-green-500',
  ...
  methods: {
    ...
    additional () {
      ...
      if (this.$attrs.columns) {
        ...
        if (this.$root.hasPermissions('new_chunk')) {
          actions.copy = this.actions.copy
        }

        if (this.$root.hasPermissions('edit_chunk')) {
          actions.disabled = this.actions.disabled
        }

        if (this.$root.hasPermissions('delete_chunk')) {
